...
Phaser.ParticleStorm = function (a, b) {
    ...
    if (Array.isArray(a))
        for (var c = 0; c < a.length; c++) this.dataList[a[c]] = this.game.cache.getJSON(a[c]);
    ...
    if (void 0 === a) this.dataList = {};
    else if (Array.isArray(a))
        for (var b = 0; b < a.length; b++) delete this.dataList[a[b]];
    else delete this.dataList[a];
    return this
}, Phaser.ParticleStorm.prototype.cloneData = function (a, b) {
    if (void 0 === a) return this;
    if (Array.isArray(a))
        for (var c = 0; c < a.length; c++) this.dataList[a[c]] = Phaser.Utils.extend(!0, this.game.cache.getJSON(a[c]));
    ...
    if (this.active)
        for (var a = 0; a < this.emitters.length; a++) this.emitters[a].enabled && !this.emitters[a].manualUpdate && this.emitters[a].update()
    ...
    createGravityWell: function (a, b, c, d, e) {
    ...
    emitDelayed: function (a, b, c, d, e) {
    ...
    emit: function (a, b, c, d) {
        ...
        if (d.xStep > 0 ? this._step.x = d.xStep : this._step.x = 0, d.yStep > 0 ? this._step.y = d.yStep : this._step.y = 0, this._delay.enabled = !1, d.delay && (this._delay.enabled = !0, "number" == typeof d.delay ? (this._delay.start = d.delay, this._delay.step = 0, this._delay.visible = !1) : (this._delay.start = d.delay.start ? d.delay.start : 0, this._delay.step = d.delay.step ? d.delay.step : 0, this._delay.visible = d.delay.visible ? !0 : !1)), d.zone)
            if (void 0 === d.random && void 0 === d.full && void 0 === d.percent || d.random) d.zone.emit(this, a, b, c, e, d.setAlpha, d.setColor);
            else if (void 0 !== d.percent || void 0 === d.full && d.random) {
            if (void 0 !== d.percent) {
                var f = 0;
            ...
            for (var g = 0; e > g; g++) this.emitParticle(a, b, c, null);
        if (d.radiate)
            for (var h = 0; h < this.batch.length; h++) this.batch[h].radiate(d.radiate.velocity, d.radiate.from, d.radiate.to);
        else if (d.radiateFrom)
            for (var h = 0; h < this.batch.length; h++) this.batch[h].radiateFrom(d.radiateFrom.x, d.radiateFrom.y, d.radiateFrom.velocity);
    ...
    forEach: function (a, b) {
        if (arguments.length <= 2)
            for (var c = 0; c < this.list.length; c++) a.call(b, this.list[c]);
        else {
            for (var d = [null], c = 2; c < arguments.length; c++) d.push(arguments[c]);
            for (var c = 0; c < this.list.length; c++) d[0] = this.list[c], a.apply(b, d)
        }
    },
    forEachNew: function (a, b) {
        if (0 !== this.batch.length)
            if (arguments.length <= 2)
                for (var c = 0; c < this.batch.length; c++) a.call(b, this.batch[c]);
            else {
                for (var d = [null], c = 2; c < arguments.length; c++) d.push(arguments[c]);
                for (var c = 0; c < this.batch.length; c++) d[0] = this.batch[c], a.apply(b, d)
            }
    },
    getParticle: function (a) {
    ...
    destroy: function () {
        ...
        for (var a = this.list.length - 1; a >= 0; a--) this.list[a].kill(), this.list.splice(a, 1);
    ...
    step: function (a, b) {
        ...
        if (this._lastPercent = this.life, this.lifespan > 0 && (this.life = Math.min(this._age / this.lifespan, 1)), b && !this.ignoreForce && (this.transform.velocity.x.value += b.x, this.transform.velocity.y.value += b.y), this.transform.step(), this.color.step(), this.onUpdate(), this.alive) {
            for (this._numToEmit += this.emitter.updateFrequency(this.emit, a, this._lastPercent, this.life); this._numToEmit >= 1;) this.emitChild();
    ...
    applyOverwrite: function (a, b) {
        return b
    },
    getChildKey: function (a) {
        if (Array.isArray(a)) return this.emitter.game.rnd.pick(a);
        if (void 0 !== a.at && a.at.length > 0) {
            for (var b = a.at[0].value, c = 0; c < a.at.length && !(a.at[c].time > this.life); c++) b = a.at[c].value;
            return b
        }
        return null
    },
    radiate: function (a, b, c) {
    ...
    target: function (a) {
        ...
        return a.speed ? (this.graph.fromControl({
            value: 2 * j,
            control: a.speed
        }, d.velocity.x), this.graph.fromControl({
            value: 2 * k,
            control: a.speed
        }, d.velocity.y)) : (d.velocity.x.value = j, d.velocity.y.value = k), this
    },
    setLife: function (a, b) {
    ...
    getControlValues: function (a, b, c) {
    ...
    emit: function (a, b, c, d, e) {
        ...
        for (var f = null, g = 0; e > g; g++) this.shape.random(this._rnd), f = a.emitParticle(b, c + this._rnd.x * this.scale.x, d + this._rnd.y * this.scale.y, null);
        return f
    }
    ...
    void 0 === b && (b = 0), void 0 === c && (c = 1e3), void 0 === d && (d = !0), Phaser.ParticleStorm.Zones.Base.call(this, a), this.math = this.game.math, this.points = {
        x: [],
        y: []
    }, this.path = [], this.resolution = c, this.mode = b, this.closed = d, this.mult = 0, this.update(e)
    ...
    for (var b = 0; b < a.length; b++) this.points.x.push(a[b].x), this.points.y.push(a[b].y);
    ...
    for (var c = 0, d = 1 / this.resolution, b = 0; 1 >= b; b += d) {
        if (0 === this.mode) var e = this.math.linearInterpolation(this.points.x, b),
            f = this.math.linearInterpolation(this.points.y, b);
        else if (1 === this.mode) var e = this.math.bezierInterpolation(this.points.x, b),
            f = this.math.bezierInterpolation(this.points.y, b);
        else if (2 === this.mode) var e = this.math.catmullRomInterpolation(this.points.x, b),
    ...
    Array.isArray(c) && (c = this.game.rnd.between(c[0], c[1])), Array.isArray(d) && (d = this.game.rnd.between(d[0], d[1]));
    for (var f = null, g = null, h = 0; e > h; h++) f = this.game.rnd.pick(this.path), g = a.emitParticle(b, c + f.x, d + f.y);
    return g
    ...
    Array.isArray(c) && (c = this.game.rnd.between(c[0], c[1])), Array.isArray(d) && (d = this.game.rnd.between(d[0], d[1]));
    for (var h = null, i = null, j = 0; e > j; j++) h = this.game.rnd.pick(this.points), i = a.emitParticle(b, c + h.x * this.scale.x, d + h.y * this.scale.y), i && (f && h.color.a < 1 && (i.color.alpha.value = h.color.a), g && i.color.setColor(h.color.r, h.color.g, h.color.b, h.color.a));
    return i
    ...
    this.particle = a, this.time = a.emitter.game.time, this.graph = Phaser.ParticleStorm.Graph, this.x = 0, this.y = 0, this.velocity = {
        x: null,
        y: null,
        facing: null
    }, this.acceleration = {
    ...
    init: function (a, b, c) {
        if (this.x = a, this.y = b, c.hasOwnProperty("anchor") ? this.anchor.set(c.anchor) : (c.hasOwnProperty("anchorX") && (this.anchor.x = c.anchorX), c.hasOwnProperty("anchorY") && (this.anchor.y = c.anchorY)), c.hasOwnProperty("velocity")) {
            if (this.graph.isNumeric(c.velocity)) this.velocity.x.value = c.velocity, this.velocity.y.value = c.velocity;
            else if (c.velocity.hasOwnProperty("min")) this.velocity.x.value = this.graph.getMinMax(c.velocity), this.velocity.y.value = this.velocity.x.value;
            else if (c.velocity.radial) {
